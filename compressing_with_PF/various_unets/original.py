...
def forward_fn(inputs):
    ...
    with tf.name_scope('layer_1'):
        conv1 = tf.layers.conv2d(inputs, 64, 3, 1, activation=tf.nn.relu, name='conv1_1', padding="same")
        conv1 = tf.layers.conv2d(conv1, 64, 3, 1, activation=tf.nn.relu, name='conv1_2', padding="same")
        pool1 = tf.layers.max_pooling2d(conv1, pool_size=(2, 2), strides=(2, 2), name='pool1', padding="valid")
    # conv2
    with tf.name_scope('layer_2'):
        conv2 = tf.layers.conv2d(pool1, 128, 3, 1, activation=tf.nn.relu, name='conv2_1', padding="same")
        conv2 = tf.layers.conv2d(conv2, 128, 3, 1, activation=tf.nn.relu, name='conv2_2', padding="same")
        pool2 = tf.layers.max_pooling2d(conv2, pool_size=(2, 2), strides=(2, 2), name='pool2', padding="valid")
    # conv3
    with tf.name_scope('layer_3'):
        conv3 = tf.layers.conv2d(pool2, 256, 3, 1, activation=tf.nn.relu, name='conv3_1', padding="same")
        conv3 = tf.layers.conv2d(conv3, 256, 3, 1, activation=tf.nn.relu, name='conv3_2', padding="same")
        pool3 = tf.layers.max_pooling2d(conv3, pool_size=(2, 2), strides=(2, 2), name='pool3', padding="valid")
    # conv4
    with tf.name_scope('layer_4'):
        conv4 = tf.layers.conv2d(pool3, 512, 3, 1, activation=tf.nn.relu, name='conv4_1', padding="same")
        conv4 = tf.layers.conv2d(conv4, 512, 3, 1, activation=tf.nn.relu, name='conv4_2', padding="same")
        pool4 = tf.layers.max_pooling2d(conv4, pool_size=(2, 2), strides=(2, 2), name='pool4', padding="valid")
    # conv5
    with tf.name_scope('layer_5'):
        conv5 = tf.layers.conv2d(pool4, 1024, 3, 1, activation=tf.nn.relu, name='conv5_1', padding="same")
        conv5 = tf.layers.conv2d(conv5, 1024, 3, 1, activation=tf.nn.relu, name='conv5_2', padding="same")
