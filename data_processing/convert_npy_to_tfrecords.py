...
def convert_whole_dataset(dataset_name='mri'):
    """
    convert all train, validate, test .npy files in dataset generated by prepare_datasets.prepare_data()
    :param dataset_name: dataset name, "mri" or "sorteo"
    :return:
    """
    if not (dataset_name == "mri" or dataset_name == "sorteo"):
        exit("❗️dataset_name can only be 'mri' or 'sorteo'")

    dataset_name = "/" + dataset_name
    data_dir = DATASET_DIR + dataset_name + "_pad_4_results/data"
    saving_dir = DATASET_DIR + dataset_name + "_pad_4_results/tfrecords"
    if not os.path.exists(saving_dir):
        print("🚩mkdir " + saving_dir)
