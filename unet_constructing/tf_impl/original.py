...
class UNet:
    ...
    def build_up_unet(self, batch_size):
        ...
        with tf.name_scope('layer_1'):
            # conv_1
            self.w[1] = self.init_w(shape=[3, 3, FLAGS.input_shape[2], 64], name='kernel', scope_name="conv1_1")
            ...
            # conv_2
            self.w[2] = self.init_w(shape=[3, 3, 64, 64], name='kernel', scope_name="conv1_2")
        ...
        with tf.name_scope('layer_2'):
            # conv_1
            self.w[3] = self.init_w(shape=[3, 3, 64, 128], name='kernel', scope_name="conv2_1")
            ...
            # conv_2
            self.w[4] = self.init_w(shape=[3, 3, 128, 128], name='kernel', scope_name="conv2_2")
        ...
        with tf.name_scope('layer_3'):
            # conv_1
            self.w[5] = self.init_w(shape=[3, 3, 128, 256], name='kernel', scope_name="conv3_1")
            conv_1_result = tf.nn.conv2d(
                input=maxpool_result, filter=self.w[5], strides=[1, 1, 1, 1], padding='SAME', name='conv3_1')
            relu_1_result = tf.nn.relu(features=conv_1_result, name='relu_1')
            # conv_2
            self.w[6] = self.init_w(shape=[3, 3, 256, 256], name='kernel', scope_name="conv3_2")
        ...
        with tf.name_scope('layer_4'):
            # conv_1
            self.w[7] = self.init_w(shape=[3, 3, 256, 512], name='kernel', scope_name="conv4_1")
            conv_1_result = tf.nn.conv2d(input=maxpool_result, filter=self.w[7], strides=[1, 1, 1, 1],
                                         padding='SAME', name='conv4_1')
            relu_1_result = tf.nn.relu(features=conv_1_result, name='relu_1')
            # conv_2
            self.w[8] = self.init_w(shape=[3, 3, 512, 512], name='kernel', scope_name="conv4_2")
        ...
        with tf.name_scope('layer_5'):
            # conv_1
            self.w[9] = self.init_w(shape=[3, 3, 512, 1024], name='kernel', scope_name="conv5_1")
            conv_1_result = tf.nn.conv2d(input=maxpool_result, filter=self.w[9], strides=[1, 1, 1, 1],
                                         padding='SAME', name='conv5_1')
            relu_1_result = tf.nn.relu(features=conv_1_result, name='relu_1')
            # conv_2
            self.w[10] = self.init_w(shape=[3, 3, 1024, 1024], name='kernel', scope_name="conv5_2")
            conv_2_result = tf.nn.conv2d(input=relu_1_result, filter=self.w[10], strides=[1, 1, 1, 1],
                                         padding='SAME', name='conv5_2')
            relu_2_result = tf.nn.relu(features=conv_2_result, name='relu_2')

            # up_sampling
            self.w[11] = self.init_w(shape=[2, 2, 512, 1024], name='kernel', scope_name="Up_Sampling_5")
            result_up = tf.nn.conv2d_transpose(value=relu_2_result, filter=self.w[11], strides=[1, 2, 2, 1],
                                               output_shape=[batch_size, int(relu_2_result.shape[1]) * 2,
                                                             int(relu_2_result.shape[2]) * 2, 512],
        ...
        with tf.name_scope('layer_8'):
            ...
            # conv_1
            self.w[18] = self.init_w(shape=[3, 3, 256, 128], name='kernel', scope_name="conv8_1")
            conv_1_result = tf.nn.conv2d(input=result_merge, filter=self.w[18], strides=[1, 1, 1, 1],
                                         padding='SAME', name='conv8_1')
            relu_1_result = tf.nn.relu(features=conv_1_result, name='relu_1')
            # conv_2
            self.w[19] = self.init_w(shape=[3, 3, 128, 128], name='kernel', scope_name="conv8_2")
            ...
            result_up = tf.nn.conv2d_transpose(value=relu_2_result, filter=self.w[20], strides=[1, 2, 2, 1],
                                               output_shape=[batch_size, int(relu_2_result.shape[1]) * 2,
                                                             int(relu_2_result.shape[2]) * 2, 64],
