...
class UNet:
    ...
    def build_up_unet(data, class_num):
        ...
        conv2 = Convolution2D(int(128 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN2)
        BN2 = BatchNormalization()(conv2)
        conv2 = Convolution2D(int(128 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN2)
        pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)

        BN3 = BatchNormalization()(pool2)
        conv3 = Convolution2D(int(256 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN3)
        BN3 = BatchNormalization()(conv3)
        conv3 = Convolution2D(int(256 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN3)
        pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)

        BN4 = BatchNormalization()(pool3)
        conv4 = Convolution2D(int(512 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN4)
        BN4 = BatchNormalization()(conv4)
        conv4 = Convolution2D(int(512 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN4)
        pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)

        BN5 = BatchNormalization()(pool4)
        conv5 = Convolution2D(int(1024 / FLAGS.divisor), 3, 3, activation='relu', border_mode='same')(BN5)
